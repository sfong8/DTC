...
from keras.layers import LSTM,Dense, Reshape, UpSampling2D, Conv2DTranspose, GlobalAveragePooling1D, Softmax
...
ts2 = np.array(ts)
ts3=np.reshape(ts2, ts2.shape + (1,))
n_clusters=None
# Load data
(X_train, y_train), (X_val, y_val) = (ts3,None), (None, None)  # no train/validation split for now
